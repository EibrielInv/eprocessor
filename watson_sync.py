...
class watson_sync:
    ...
    def watson_diff(self, bot_script):
        ...
        for script_intent in bot_script.get_intents():
            exists_intent = False
            for watson_intent in watson_intents["intents"]:
                if script_intent == watson_intent["intent"]:
                    exists_intent = True
                    for script_example in bot_script.get_intents()[script_intent]["default"]["es"]:
            ...
            if exists_intent:
                ...
                intent_to_add = {
                    "intent_id": script_intent,
                    "intent_data": bot_script.get_intents()[script_intent]
                }
    ...
    def sync_intents(self, diff):
        ...
        for intent in diff["intents_to_remove"]:
            response = self.conversation.delete_intent(
                       workspace_id=self.workspace_id,
                       intent=intent["intent_id"],
            )
    ...
    def send_message(self, input_text):
        watson_analysis = self.conversation.message(
            workspace_id=self.workspace_id,
            message_input={
                'text': input_text
            }
        )
        return watson_analysis
